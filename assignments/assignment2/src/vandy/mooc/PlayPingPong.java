 ...
 * @class PlayPingPong
 * 
 * @brief This class uses elements of the Android HaMeR framework to create two
 *        Threads that alternately print "Ping" and "Pong", respectively, on the
 *        display.
 */
public class PlayPingPong implements Runnable {
    ...
    /**
     * Define a CyclicBarrier synchronizer that ensures the HandlerThreads are
     * fully initialized before the ping-pong algorithm begins.
     */
    // @@ TODO - you fill in here.
    private CyclicBarrier barrier = new CyclicBarrier(2);

    /**
     * Implements the concurrent ping/pong algorithm using a pair of Android
     * Handlers (which are defined as an array field in the enclosing
     * PlayPingPong class so they can be shared by the ping and pong objects).
     * The class (1) extends the HandlerThread superclass to enable it to run in
     * the background and (2) implements the Handler.Callback interface so its
     * handleMessage() method can be dispatched without requiring additional
     * subclassing.
     */
    class PingPongThread extends HandlerThread implements Handler.Callback {
        /**
         * Keeps track of whether this Thread handles "pings" or "pongs".
         */
        ...
        /**
         * Constructor initializes the superclass and type field (which is
         * either PING or PONG).
         */
        public PingPongThread(PingPong myType) {
            ...
            // @@ TODO - you fill in here.
            mMyType = myType;
            Log.d(TAG, "mMyType=" + mMyType);
        }

        /**
         * This hook method is dispatched after the HandlerThread has been
         * started. It performs ping-pong initialization prior to the
         * HandlerThread running its event loop.
         */
        @Override
        protected void onLooperPrepared() {
            ...
            // @@ TODO - you fill in here.
            handlers[mMyType.ordinal()] = new Handler(this);

            try {
                // Wait for both Threads to initialize their Handlers.
                // @@ TODO - you fill in here.
                Log.d(TAG, "before barrier, mMyType=" + mMyType);
                barrier.await();
                Log.d(TAG, "after barrier, mMyType=" + mMyType);
            } catch (Exception e) {
                e.printStackTrace();
            }
            ...
            if (mMyType == PingPong.PONG) {
                Message message = Message.obtain(
                        handlers[PingPong.PING.ordinal()],
                        PingPong.PING.ordinal(),
                        handlers[PingPong.PONG.ordinal()]);
                message.sendToTarget();
            }
        }

        /**
         * Hook method called back by HandlerThread to perform the ping-pong
         * protocol concurrently.
         */
        @Override
        public boolean handleMessage(Message reqMsg) {
            Log.d(TAG, "handleMessage, mMyType=" + mMyType);
            ...
            // the reply.
            // @@ TODO - you fill in here.
            Message message = Message.obtain((Handler) reqMsg.obj,
                    mMyType == PingPong.PING ? PingPong.PONG.ordinal()
                            : PingPong.PING.ordinal(), handlers[mMyType
                            .ordinal()]);
     ...
     * method in a Java class, an Android Activity, etc.
     */
    public void run() {
