...
public class PlayPingPong implements Runnable {
    ...
    Handler mHandlerPing;
    Handler mHandlerPong;
     ...
     */
    // @@ TODO - you fill in here.
    CyclicBarrier mCyclicBarrier = new CyclicBarrier(2);
    ...
    class PingPongThread extends HandlerThread implements Handler.Callback {
        ...
        protected void onLooperPrepared() {
            ...
            if (mMyType == PingPong.PING)
                mHandlerPing = new Handler(getLooper(), this);
            else
                mHandlerPong= new Handler(getLooper(), this);
            ...
            // @@ TODO - you fill in here.
            if (mMyType == PingPong.PING) {
                Message m = mHandlerPing.obtainMessage();
                m.setTarget(mHandlerPing);
                m.obj = mHandlerPong;
                mHandlerPing.sendMessage(m);
            }
        }
        ...
        public boolean handleMessage(Message reqMsg) {
            ...
            if (++mIterationsCompleted <= mMaxIterations) {
                mOutputStrategy.print(mMyType.name()+" " + mIterationsCompleted + "\n");
            } else {
                // Shutdown the HandlerThread to the main PingPong
                // thread can join with it.
                // @@ TODO - you fill in here.
                getLooper().quit(); // or quit;
            }
            ...
            // the reply.
            // @@ TODO - you fill in here.
            Message m = reqMsg.getTarget().obtainMessage();
            m.setTarget((Handler) reqMsg.obj);
            m.obj = reqMsg.getTarget();
            ...
            if (((Handler) reqMsg.obj).getLooper().getThread().isAlive())
                m.sendToTarget(); //  m.getTarget().sendMessage(m);
